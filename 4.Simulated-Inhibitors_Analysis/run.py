...
if __name__ == "__main__":
    ...
    for replicate in ['test_A','test_B','test_C']:

        for f in os.listdir('./simulated_data/replicate_tests/'+replicate+'/simulated_data/'):

            if f[0] != '.' and f[0] != "~":

                n_inhib = f.split('_')[2]
                test_type = '_'.join(f.split('.')[0].split('_')[5:])
                dataset = pd.read_excel('./simulated_data/replicate_tests/'+replicate+'/simulated_data/'+f, engine='openpyxl',  nrows=int(n_inhib))
                dataset.name = replicate+'_'+str(n_inhib)+'i_'+test_type
    ...
    for x in range(0,5):
        ...
        for dataset in datasets:
            ...
            for mu in [200,700]:
                ...
                output_name = str(dataset.name) + '_mu_' + str(mu) +'_' + str(x)

                print('Working on...')
                print(output_name)
                
                new_prior_settings = (prior_type, (bdist_alpha, bdist_beta, mu, size))
                JS.main_function(dataset, prior_settings, noise_variance, max_combination_iter, influence, output_name, num_p, on_target_inhibition_threshold, number_top_UW_scores_to_maximize)
