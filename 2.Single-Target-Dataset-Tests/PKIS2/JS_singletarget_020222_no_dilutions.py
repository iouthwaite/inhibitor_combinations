...
# Returns the inhibitor indicies that target each kinase - if there are MORE THAN (X) INHIBITORS, ignore that kinase target for the purposes of these tests ############################################################################################################################################
def get_i_target_k(data,otit):
    res = []
    for k in range(0,len(data[0])):
        i_data = data[:,k]
        target_i = [i for i,val in enumerate(i_data) if val >= otit]
        if len(target_i) <= 700: #set at 700 to use all inhibitors in PKIS2, possible to reduce if desired
            res.append(target_i)
        else:
            res.append([])
    return res
...
def get_combinations(i_target_k, cmbn, data, otit):
    res = []
    for k in range(0, len(i_target_k)):
        ...
        for c in itertools.combinations(inhibs,cmbn):
            ...
            for i in c:
                target = data[i,k]
                concentrations.append(1) #DO NOT DILUTE THE INHIBITORS FOR THIS SPECIFIC SCRIPT VERSION, use all initially at 1uM ###################
                on.append(target)
                off.append((i,k))
            on = np.array(on)
...
def JS_UW(inpt):
    ...
    off_target_pointers = combination[3]
    concentrations = combination[4]

    #unpack the off-target values from the data array into the actual off-target values defined by the selector
    off_target_vals = []

    for pset in off_target_pointers:
        i = pset[0]
        k = pset[1]
        off_target_selector = [x for x in range(data.shape[1]) if x != k]
        off_target_vals.append(data[i, off_target_selector])

    off_target_vals = np.array(off_target_vals)

    #print(off_target_vals)
...
def opt_w(on_target_values, off_target_values, prior, noise_variance, influence, num_w, concentrations, otit):
    ...
    #in this script version do NOT optimize concentrations ################
...
def main_function(dataset,prior_settings,noise_variance,max_combination_iter,influence,output_name,num_p,otit,number_top_UW_scores_to_maximize):
    ...
    for x in range(1,max_combination_iter+1):
        ...
        #build a list of the indicies to score - note, in this version of the script, we ignore those kinases with >50 possible inhibitors, in order to speed up the program in order to generate meaningful possible results at higher combination numbers
        i_target_k = get_i_target_k(data,otit)

        combos = get_combinations(i_target_k, x, data, otit)
        ...
        t_r = len(combos)
        p_input = list(zip(combos, repeat(on_target_prior), repeat(noise_variance), repeat(influence), num_r, repeat(t_r), repeat(curr_t), repeat(data)))
        ...
        print('Aggregated unweighed initial scores')

        top_scores = []
        for k in temp_UW:
            #get the single top score from the set of results for each kinse - do NOT sort here, since that would take too long. Just do a single pass.
            topscore = 0
            topscore_index = 0
            if k != []:
                for i in range(0, len(k)):
                    ...
                    if res[2] > topscore:
                        topscore=res[2]
                        topscore_index = i
                top_scores.append(k[topscore_index])
...
if __name__ == '__main__':
    ...
    dataset=pd.read_excel("./my_dataset_etc.xlsx", engine='openpyxl', nrows=39, usecols=range(0,328))
